...
class TestmicroLens(unittest.TestCase):
	...
	def test_get_centroid_shift_luminos_lens(self):
		...
		self.assertAlmostEqual(m.get_centroid_shift_unresolved(0.3,57.7,6.507*10.7,lensMag=15.3,sourceMag=18.5),0.030,places=2)	
		self.assertAlmostEqual(m.get_centroid_shift_unresolved(0.3,26.1,5.2*9.678,lensMag=13.6,sourceMag=19.6),0.007,places=2)
		self.assertAlmostEqual(m.get_centroid_shift_unresolved(0.3,169.3,3.797*32.9,lensMag=17.6,sourceMag=16.2),0.092,places=2)
		self.assertAlmostEqual(m.get_centroid_shift_unresolved(0.55,192.3,4.824*26.7,lensMag=14.9,sourceMag=17.0),0.022,places=2)
		self.assertAlmostEqual(m.get_centroid_shift_unresolved(0.6,5.6,29.560*4.5,lensMag=12.1,sourceMag=19.7),0.006,places=2)
		self.assertAlmostEqual(m.get_centroid_shift_unresolved(0.35,19.3,12.141*5.0,lensMag=12.1,sourceMag=18.7),0.005,places=2)
		#self.assertAlmostEqual((m.get_centroid_shift_unresolved(0.45,42.9,9.243*4.0,lensMag=12.7,sourceMag=12.7),1.083,places=1)
